...
- **`fit start` 命令启动方式**：
  ...
  - 如果需要使用 Nacos 版注册中心，操作步骤：
    ...
    2. **放入 build/plugins 目录**：
       ```bash
       cp fit-builtin/plugins/fit-service-coordination-nacos/target/fit-service-coordination-nacos-3.6.2-SNAPSHOT.jar ../../../build/plugins/
       ```
    3. **移除 Simple 插件**：
       ```bash
       rm ../../../build/plugins/fit-service-coordination-simple-3.6.2-SNAPSHOT.jar
       ```
